 ...
 * create Mar 13, 2017
 */
@Repository(value = SpringBeanNames.REPOSITORY_NAME_P300BREP)
@Scope("prototype")
public class P300BRePMGDaoImpl extends QbMongoDaoSupport implements P300BRePMGDao {
	@Override
	public List<Pieper> getListPieper(double pnLa, double pnLong, int fo100, int fo100f, String pvSearch, String pvSearchStem, int sort, int ft310, int ft320, int ft330, int pnOffset, int pnLimit) {
		// TODO Auto-generated method stub
		try {
			setFunctionName(QbMongoFunctionNames.P300B_LISTOFTABP300BRE);
			...
			setParameterNumber(ft310);
			setParameterNumber(ft320);
			setParameterNumber(ft330);
			...
			if (kq != null && ApplicationHelper.isJSONArray(kq)) {
	...
	public P300BRePMG getPieperDetail(int fo100, int pp100) {
		try {
			setFunctionName(QbMongoFunctionNames.P300B_LISTOFTABP300B_DETAIL_RE);
			setParameterNumber(fo100);
			setParameterNumber(pp100);
			String kq = executeFunction(ApplicationConstant.DB_TYPE_PIEPME, fo100)
					.toString();
			if (kq != null) {
				P300BRePMG p300pmg = new P300BRePMG();
				...
				p300pmg.setFt310(json.getInt("FT310"));
				p300pmg.setFt320(json.getInt("FT320"));
				p300pmg.setFt330(json.getInt("FT330"));
				...
				if(json.has("TV311"))
					p300pmg.setTv311(json.getString("TV311"));
				if(json.has("TV321"))
					p300pmg.setTv321(json.getString("TV321"));
				if(json.has("TV333"))
					p300pmg.setTv333(json.getString("TV333"));
				if(json.has("PD316"))
					p300pmg.setPd316(df.parse(json.getString("PD316")));
				if(json.has("PN307"))
					p300pmg.setPn307(json.getInt("PN307"));
				if(json.has("KV105"))
					p300pmg.setKv105(json.getString("KV105"));
				return p300pmg;
			}
			return null;
		} catch (Exception ex) {
