...
(function(factory) {
	if (typeof define === 'function' && define.amd) {
		define(
				[ 'jquery', 'lodash', 'jquery-ui/core', 'jquery-ui/widget',
						'jquery-ui/mouse', 'jquery-ui/draggable',
		...
		(function($, _) {
			...
			var GridStackEngine = function(width, onchange, float_mode, height,
					items) {
				this.width = width;
			...
			GridStackEngine.prototype._fix_collisions = function(node) {
				...
				var nn = node, has_locked = Boolean(_.find(this.nodes,
						function(n) {
							return n.locked
						}));
				...
				while (true) {
					...
					this.move_node(collision_node, collision_node.x, node.y
							+ node.height, collision_node.width,
							collision_node.height, true);
				}
			};

			GridStackEngine.prototype.is_area_empty = function(x, y, width,
					height) {
				var nn = {
					x : x || 0,
					y : y || 0,
			...
			GridStackEngine.prototype._pack_nodes = function() {
				...
				if (this.float) {
					_.each(this.nodes, function(n, i) {
						if (n._updating || typeof n._orig_y == 'undefined'
								|| n.y == n._orig_y)
							return;

						var new_y = n.y;
						while (new_y >= n._orig_y) {
							var collision_node = _.chain(this.nodes).find(
									function(bn) {
										return n != bn
			...
			GridStackEngine.prototype._notify = function() {
				...
				var deleted_nodes = Array.prototype.slice.call(arguments, 1)
						.concat(this.get_dirty_nodes());
			...
			GridStackEngine.prototype.can_move_node = function(node, x, y,
					width, height) {
				...
				var cloned_node;
				var clone = new GridStackEngine(this.width, null, this.float,
						0, _.map(this.nodes, function(n) {
							if (n == node) {
				...
				if (has_locked)
					res &= !Boolean(_.find(clone.nodes, function(n) {
						return n != cloned_node && Boolean(n.locked)
								&& Boolean(n._dirty);
					}));
				if (this.height)
			...
			GridStackEngine.prototype.can_be_placed_with_respect_to_height = function(
					node) {
				if (!this.height)
					return true;
			...
			GridStackEngine.prototype.move_node = function(node, x, y, width,
					height, no_pack) {
				if (typeof x != 'number')
					x = node.x;
				if (typeof y != 'number')
				...
				if (typeof node.max_width != 'undefined')
					width = Math.min(width, node.max_width);
				if (typeof node.max_height != 'undefined')
					height = Math.min(height, node.max_height);
				if (typeof node.min_width != 'undefined')
					width = Math.max(width, node.min_width);
				if (typeof node.min_height != 'undefined')
					height = Math.max(height, node.min_height);

				if (node.x == x && node.y == y && node.width == width
						&& node.height == height) {
					return node;
				}
			...
			var GridStack = function(el, opts) {
				...
				var is_nested = this.container.closest('.' + opts.item_class)
						.size() > 0;
				...
				this.opts = _
						.defaults(
								opts || {},
								{
									width : parseInt(this.container
											.attr('data-gs-width')) || 12,
									height : parseInt(this.container
									...
									_class : 'grid-stack-'
											+ (Math.random() * 10000)
													.toFixed(0),
									animate : Boolean(this.container
											.attr('data-gs-animate')) || false,
									always_show_resize_handle : opts.always_show_resize_handle || false,
									resizable : _.defaults(
											opts.resizable || {}, {
												// autoHide:
												// !(opts.always_show_resize_handle
												// || false),
												handles : 'se'
											}),
									draggable : _
						...
						function(nodes) {
							...
							_.each(nodes, function(n) {
								if (n._id == null) {
									...
									n.el.attr('data-gs-x', n.x).attr(
											'data-gs-y', n.y).attr(
											'data-gs-width', n.width).attr(
											'data-gs-height', n.height);
				...
				if (this.opts.auto) {
					...
					this.container.children(
							'.' + this.opts.item_class + ':not(.'
									+ this.opts.placeholder_class + ')').each(
							function(index, el) {
								el = $(el);
								elements.push({
									el : el,
									i : parseInt(el.attr('data-gs-x'))
											+ parseInt(el.attr('data-gs-y'))
											* _this.opts.width
								});
							});
				...
				this.placeholder = $(
						'<div class="' + this.opts.placeholder_class + ' '
								+ this.opts.item_class + '">'
								+ '<div class="placeholder-content" /></div>')
						.hide();

				this.container.height(this.grid.get_grid_height()
			...
			GridStack.prototype._init_styles = function() {
				...
				this._styles_id = 'gridstack-style-'
			...
			GridStack.prototype._update_styles = function(max_height) {
				...
				if (this._styles._max == 0) {
					Utils.insert_css_rule(this._styles, prefix, 'min-height: '
							+ (this.opts.cell_height) + 'px;', 0);
				}
				...
				if (max_height > this._styles._max) {
					for (var i = this._styles._max; i < max_height; ++i) {
						Utils
								.insert_css_rule(
										...
										prefix + '[data-gs-height="' + (i + 1)
												+ '"]',
										'height: '
												+ (this.opts.cell_height
														* (i + 1) + this.opts.vertical_margin
														* i) + 'px;', i);
						Utils
								.insert_css_rule(
										this._styles,
										prefix + '[data-gs-min-height="'
												+ (i + 1) + '"]',
										'min-height: '
												+ (this.opts.cell_height
														* (i + 1) + this.opts.vertical_margin
						...
						// prefix + '[data-gs-y="' + i + '"]',
						// 'margin-top: ' + (this.opts.cell_height * i +
						// this.opts.vertical_margin * i) + 'px;',
						// i
						// );
					}
			...
			GridStack.prototype._is_one_column_mode = function() {
				return (window.innerWidth
						|| document.documentElement.clientWidth || document.body.clientWidth) <= this.opts.min_width;
			};
			...
			GridStack.prototype._prepare_element = function(el) {
				...
				var on_start_moving = function(event, ui) {
					...
					cell_width = Math.ceil(o.outerWidth()
							/ o.attr('data-gs-width'));
					cell_height = self.opts.cell_height
							+ self.opts.vertical_margin;
					// self.placeholder
					...
					el.resizable('option', 'minWidth', cell_width
							* (node.min_width || 1));
					el.resizable('option', 'minHeight', self.opts.cell_height
							* (node.min_height || 1));
				};
				...
				var on_end_moving = function(event, ui) {
					...
					o.attr('data-gs-x', node.x).attr('data-gs-y', node.y).attr(
							'data-gs-width', node.width).attr('data-gs-height',
							node.height).removeAttr('style');
								...
								.extend(
										this.opts.resizable,
										{
											...
											resize : function(event, ui) {
												var x = Math
														.round(ui.position.left
																/ cell_width), y = Math
														.floor((ui.position.top + cell_height / 2)
																/ cell_height), width = Math
														.floor(ui.size.width
																/ cell_width) + 1, height = Math
														.round(ui.size.height
																/ cell_height);
												if (!self.grid.can_move_node(
														node, x, y, width,
														height)) {
													return;
												}
			...
			GridStack.prototype.add_widget = function(el, x, y, width, height,
				...
				if (typeof auto_position != 'undefined')
					el.attr('data-gs-auto-position', auto_position ? 'yes'
							: null);
				...
				this.container.sortable({
					handle : '.item.btn-draggable',
					containment : 'parent',
					tolerance : 'intersect',
					//axis: 'x'
				});
			
//				this.container.dragswap({
...
//				});
				...
				// grid.refreshGridLineY(this.container);
				// grid.setUpGridLine(this.container);
				return el;
			};

			GridStack.prototype.will_it_fit = function(x, y, width, height,
					auto_position) {
				var node = {
					x : x,
					y : y,
			...
			GridStack.prototype.remove_widget = function(el, detach_node) {
				detach_node = typeof detach_node === 'undefined' ? true
						: detach_node;
				el = $(el);
			...
			GridStack.prototype.disable = function() {
				this.movable(this.container
						.children('.' + this.opts.item_class), false);
				this.resizable(this.container.children('.'
						+ this.opts.item_class), false);
			};

			GridStack.prototype.enable = function() {
				this.movable(this.container
						.children('.' + this.opts.item_class), true);
				this.resizable(this.container.children('.'
						+ this.opts.item_class), true);
			};
			...
			GridStack.prototype.resize = function(el, width, height) {
				this
						._update_element(
								el,
								function(el, node) {
									width = (width != null && typeof width != 'undefined') ? width
											: node.width;
									height = (height != null && typeof height != 'undefined') ? height
											: node.height;

									this.grid.move_node(node, node.x, node.y,
											width, height);
								});
			};
			...
			GridStack.prototype.update = function(el, x, y, width, height) {
				this
						._update_element(
								el,
								function(el, node) {
									x = (x != null && typeof x != 'undefined') ? x
											: node.x;
									y = (y != null && typeof y != 'undefined') ? y
											: node.y;
									width = (width != null && typeof width != 'undefined') ? width
											: node.width;
									height = (height != null && typeof height != 'undefined') ? height
									...
									this.grid.move_node(node, x, y, width,
											height);
								});
			};
			...
			GridStack.prototype.cell_width = function() {
				var o = this.container.children('.' + this.opts.item_class)
						.first();
			...
			GridStack.prototype.get_cell_from_pixel = function(position) {
				...
				var column_width = Math.floor(this.container.width()
						/ this.opts.width);
				var row_height = this.opts.cell_height
						+ this.opts.vertical_margin;
