(function() {
	...
	supportsCanvas = !!(supportsCanvas.getContext && supportsCanvas
			.getContext('2d'));
	// helper functions
	...
	function factory($) {
		function Crop($image, options) {
			...
			this.$image = $image;
			this.$image.hide().prop('draggable', false).addClass('cropImage')
					.wrap('<div class="cropFrame" />'); // wrap
			// image
			// in
			// frame;
		...
		Crop.prototype = {
			init : function() {
				var self = this;
				cropControlsNote = $('<div/>', {
					'class' : 'cropControlsNote'
				})
				
				if(!self.options.isDialog){
					cropControlsNote.append($('<i class="fa fa-arrows"></i>'));
					
					cropControlsNote.append($('<span/>', {
						'text' : getLocalize("drop_image_drag")
					}));
				}
				...
				if(!self.options.isDialog){
					var defaultControls = $('<div/>', {
						'class' : 'cropControls'
					}).append($('<span>' + this.options.label + '</span>')).append(
							$('<button/>', {
								'class' : 'cutImage',
								'type' : 'button',
								//'text' : getLocalize("save")
							})).append(cropControlsNote
									).append(
									$('<button/>', {
										'class' : 'cropCancel',
										'type' : 'button',
										//'text' : getLocalize("cancel")
							}));
				}else{
					var defaultControls = $('<div/>', {
						'class' : 'cropControls'
				  ...
				  /*.append($('<button/>', {
							'class' : 'cropZoomIn',
							'type' : 'button'
						}).on('click', $.proxy(this.zoomIn, this))).append(
						$('<button/>', {
							'class' : 'cropZoomOut',
							'type' : 'button'
				...
				if(!self.options.isDialog){
					$('.cutImage').append('<i class="fa fa-crop"></i>');
					$('.cropCancel').append('<i class="fa fa-times"></i>');
				}
				...
				if ($.fn.mousewheel) {
					$(document).on('mousewheel.' + pluginName,this.$image, function(e) {
			...
			remove : function() {
				...
				if (hammerit)
					hammerit
							.off('mousedown dragleft dragright dragup dragdown release doubletap pinchin pinchout');
			...
			fit : function() {
				var widthRatio = this.options.width / this.width, heightRatio = this.options.height
						/ this.height;
				this.minPercent = (widthRatio >= heightRatio) ? widthRatio
						: heightRatio;
			},
			setCrop : function(result) {
				this.percent = Math.max(this.options.width / result.cropW,
						this.options.height / result.cropH);
			...
			zoom : function(percent) {
				...
				this.percent = Math.max(this.minPercent, Math.min(
						this.options.maxZoom, percent));
				this.img_width = Math.ceil(this.width * this.percent);
				this.img_height = Math.ceil(this.height * this.percent);
				if (old_percent) {
					var zoomFactor = this.percent / old_percent;
					this.img_left = fill((1 - zoomFactor) * this.options.width
							/ 2 + zoomFactor * this.img_left, this.img_width,
							this.options.width);
					this.img_top = fill((1 - zoomFactor) * this.options.height
							/ 2 + zoomFactor * this.img_top, this.img_height,
							this.options.height);
				} else {
					console.log('old_percent ' + old_percent);
					this.img_left = fill(
							(this.options.width - this.img_width) / 2,
							this.img_width, this.options.width);
					this.img_top = fill(
							(this.options.height - this.img_height) / 2,
							this.img_height, this.options.height);
				}
				
				console.log('width ' + this.img_width);
			...
			zoomIn : function() {	
					this.zoom(this.percent + (1 - this.minPercent)
			...
			drag : function(data, skipupdate) {
				this.img_left = fill(data.startX + data.dx, this.img_width,
						this.options.width);
				this.img_top = fill(data.startY + data.dy, this.img_height,
						this.options.height);
				this.$image.css({
					left : this.img_left,
					top : this.img_top
			...
			getDataURL : function() {
				if (!supportsCanvas) {
					// return an empty string for browsers that don't support
					// canvas.
				...
				ctx.drawImage(this.$image.get(0), this.result.cropX,
						this.result.cropY, this.result.cropW,
						this.result.cropH, 0, 0, this.options.width,
						this.options.height);
		...
		$.fn[pluginName] = function(options) {
			return this.each(function() {
				...
				if (!inst) {
					var opts = $.extend({}, $.fn[pluginName].defaultOptions,
							options);
