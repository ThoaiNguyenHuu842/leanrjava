	...
	bigbox = {
		...
		eventListener : function() {
			...
			$(document).on('click', '.content-box .btn-upload-big-box', function(event) {
				...
				// ANPH: new function
				backgroundSrc.setBackground(targetBox, 'B');
			});
			...
			/*
			 * ThoaiNH - 29/07/2016 onclick add responsive box
			 */
			$(document).on('click', '#dia-add-box .btn-add-box-re', function() {
				var reType = $(this).attr('type');
				bigbox.onClickAddBox();
				var boxData = {
					id : bigBoxModelManager.getNewId(),
					h : 20,
		...
		initForBox : function(boxObject, boxData) {
			 ...
			 */
			boxObject.find('.grid-stack-re').initBoxRe();
			// set css
			if (boxData[WEB_PRO_CSS])
			...
			if (PAGE_TOPIC_CONTANTS != 0) {
				boxObject.find('.grid-stack-re').css({
		...
		add : function(boxData) {
			...
			var heightDt = $('#content-wrapper .qb-body-box').last().height();
			$('#content-wrapper .qb-body-box').last().find('.grid-stack-re').height(heightDt);
			// alert($('#content-wrapper
			// .qb-body-box').last().find('.grid-stack-re').height());
			// reset sortable
		 ...
		 * 
		 */
		onAfterAddComp : function(widget) {
			/*
			 * changed height of box to fix component
			 */
//			var boxId = widget.parents('.content-box').attr('qb-box-id');
//			var y = parseInt(widget.attr('data-gs-y'));
//			var height = parseInt(widget.attr('data-gs-height'));
//			var heightComp = (y + height) * gridConfig.cell_height;
//			if (($('li[qb-box-id="' + boxId + '"] div.grid-stack-re .grid-stack').size()) > 1) {
//				if (heightComp >= (widget.parents('.grid-stack').height())) {
//					widget.closest(".grid-stack-re").height(heightComp);
//					widget.closest(".grid-stack-re").find('.grid-stack').each(function(index) {
//						var setheight = $(this);
//						if ($(this).html() != (widget.parents('.grid-stack').html())) {
//							//để height(heightComp) sẽ hiển thị ra kết quả lớn hơn 2 => height(heightComp-2)
//							setheight.height(heightComp-2);
//						}
//					});
//				}
//			}
