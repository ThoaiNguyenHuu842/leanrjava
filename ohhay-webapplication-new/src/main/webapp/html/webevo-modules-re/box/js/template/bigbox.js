function buildTemplateBigBox(boxData) {
	var subBoxHTml = '';
	var type = boxData.reType;
	if(type == 1)
		subBoxHTml = '<div index="1" class="subbox-1 grid-stack"></div>';
	else
	{
		for(i=0;i<type;i++)
			subBoxHTml += '<div index="'+(i+1)+'" class="grid-stack subbox-'+type+'"></div>';
	}	
...
//	template_big_box += '		</div>';
	template_big_box += '	<div class="grid-stack-re" style="margin: auto;width: 1170px;">';
	template_big_box += '		<div class="qb-gird-handle-custom qb-edit-tool">';
	template_big_box += '			<img src="' + $("#contextPath").val() + '/html/images/move-box.png"/>';
	template_big_box += '		</div>';
	template_big_box += subBoxHTml;
	template_big_box += '		</div>';
	template_big_box += '	</div>';	
	template_big_box += '	';
	...
	template_big_box += '<li class="btn-upload-big-box"><a><i class="fa fa-picture-o"></i>' + getLocalize("web_edit_changebg") + '</a></li>';
	template_big_box += '<li class="btn-box-option"><a><i class="fa fa-cog"></i>' + getLocalize("weh_object_style") + '</a></li>';
	...
	template_big_box += '<li class="btn-set-anchor-box border-top"><a><i class="fa fa-anchor"></i><label>' + boxName + '</label></a></li>';
...
function buildTemplateBigBoxHeader(boxData) {
	...
	var boxName = boxData.name != null ? boxData.name : boxData.id;
	var libHtml = getLibHTML(boxData);
	// widget name of this box (Ex: regis-webinaris)
	var widgetAttr = "";
	if (boxData[WEB_PRO_WIGETNAME] && boxData[WEB_PRO_WIGETNAME].length > 0)
	...
	template_big_box += '<li class="btn-box-option"><a><i class="fa fa-cog"></i>' + getLocalize("weh_object_style") + '</a></li>';
	template_big_box += libHtml;
	template_big_box += '<li class="btn-set-anchor-box"><a><i class="fa fa-anchor"></i><label>' + boxName + '</label></a></li>';
