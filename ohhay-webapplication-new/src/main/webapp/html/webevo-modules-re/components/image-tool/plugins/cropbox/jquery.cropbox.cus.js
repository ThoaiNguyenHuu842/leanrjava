(function() {
	...
	function factory($) {
		function Crop($image, options) {
			...
			this.$image = $image;
			this.$image.hide().prop('draggable', false).addClass('cropImage').wrap('<div class="cropFrame" />'); // wrap
			// image
			// in
			// frame;
			...
			init : function() {
				...
				if (!self.options.isDialog) {
					var defaultControls = $('<div/>', {
						'class' : 'cropControls'
					}).append($('<span>' + this.options.label + '</span>')).append($('<button/>', {
						'class' : 'cutImage',
						'type' : 'button',
					// 'text' : getLocalize("save")
					})).append(cropControlsNote).append($('<button/>', {
						'class' : 'cropCancel',
						'type' : 'button',
				...
				/*
				 * .append($('<button/>', { 'class' : 'cropZoomIn', 'type' :
				 * 'button' }).on('click', $.proxy(this.zoomIn, this))).append(
				 * $('<button/>', { 'class' : 'cropZoomOut', 'type' : 'button'
