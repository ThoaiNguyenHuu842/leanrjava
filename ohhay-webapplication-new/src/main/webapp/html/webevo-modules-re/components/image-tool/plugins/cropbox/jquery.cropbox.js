		...
		Crop.prototype = {
			init : function() {
				...
				var defaultControls = $('<div/>', {
					'class' : 'cropControls'
				}).append($('<span>Drag to crop</span>')).append($('<a/>', {
					'class' : 'cropZoomIn'
				}).on('click', $.proxy(this.zoomIn, this))).append($('<a/>', {
					'class' : 'cropZoomOut'
				...
				if (typeof $.fn.hammer === 'function' || typeof Hammer !== 'undefined') {
					...
					hammerit.on('touch', function(e) {
						...
						// customize thêm if (!status_drag) {
						var status_drag = self.$image.data('drag');
						if (!status_drag) {
							if (!dragData)
								dragData = {
			...
			getDataURL : function(contentType) {
				...
				return canvas.toDataURL(contentType, 0.99);
			},
			getBlob : function() {
				return uri2blob(this.getDataURL());
			},
			disableDrag : function(status) {
				this.$image.data('drag', status);
			}
		};
