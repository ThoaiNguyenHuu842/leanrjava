...
(function(factory) {
    ...
    var Utils = {
        is_intercepted: function(a, b) {
        	try {
        		var boxId = b.el.parents('.content-box ').attr('qb-box-id');
            	//not process when off smart layout or when load web
            	if((bigBoxModelManager.listBox[boxId]['SMART_LAYOUT'] && bigBoxModelManager.listBox[boxId]['SMART_LAYOUT'] == 'OFF') || componentModelManager['ROLE_UPDATE'] == false)
            	{
            		return false;
            	}
    ...
    GridStack.prototype.add_widget = function(el, x, y, width, height, auto_position) {
    	//ThoaiNH; custom for copy to another box function
    	if(x < 0 && y < 0)
    		auto_position = true;
        el = $(el);
