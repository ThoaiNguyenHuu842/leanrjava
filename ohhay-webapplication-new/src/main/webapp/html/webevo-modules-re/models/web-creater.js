	...
	webCreater = {
		...
		createLayout : function() {
			...
			if(webConfig.data.imgLazy==1)
				$('.qb-panel-web-config #checkbox-lazyload').prop('checked', true);
			...
			if (bigBoxModelManager.length() > 0) {
				...
				setTimeout(function() {
					for ( var id in bigBoxModelManager.listBox) {
						...
						var h = boxData.h;
						if (web.MOBILE_EDITOR == 'on' && boxData[WEB_PRO_MDATA] && boxData[WEB_PRO_MDATA][WEB_PRO_H])
						{
						...
						$('#content-wrapper .content-box[qb-box-id="' + boxData.id + '"] .grid-stack-re').height(h * gridConfig.cell_height);
