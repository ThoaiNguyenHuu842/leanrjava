...
(function() {
	smallbox = {
		...
		eventListener : function() {
			...
			$(document).on('click', '.grid-stack-item-content .btn-set-onload-effect', function() {
				...
				animationOption.open({
					...
					callBack : function(result) {
						...
						} else if (result.type == 'removeEffect') {
							// remove Effect
							var crEffect = component.attr("onload-effect");
			...
			$(document).on('click', '.grid-stack-item-content .btn-del', function() {
				...
				var parent = self.closest('.grid-stack');
				util.confirmDialog(getLocalize("jsstd_title1"), function() {
					...
					/*
					 * ThoaiNH(02/11/2016): BUG - khong goi grid.remove_widget nen set height cho box bi sai
					 * 
					 * var dataDetach = self.detach();
					 * replace event remove.apply(self) to self.detach()
					 * ThoaiVt undo and redo after delete component 30/08/2016
					 
					try {
						undoMgr.add({
							undo : function() {
								parent.prepend(dataDetach);
							},
							redo : function() {
								self.detach();
							}
			...
			$(document).on('click', '.grid-stack-item-content .btn-set-background', function(event) {
				...
				// ANPH: new function
				backGroundSmallBox.setBackground(targetBox, type);
			});
			...
			$(document).on('click', '.grid-stack-item-content .btn-comp-copy', function() {
				var compId = $(this).closest('.grid-stack-item-content').attr('qb-component-id');
				smallbox.CLIPBOARD_ID = compId;
			});
		},
		/*
		 * remove small box
	...
	keyListener : function(e) {
		...
		if ([ 46 ].indexOf(e.keyCode) > -1 && typeof (focusingItem) != 'undefined' && focusingItem != null && typeof (focusingItem.attr('class')) != 'undefined') {
			...
			// delete key
			case 46:
				// do not delete otext-component when hover editing
				if(!$('#qb-editor').hasClass('open') && !focusingItem.hasClass('qb-component-text'))
