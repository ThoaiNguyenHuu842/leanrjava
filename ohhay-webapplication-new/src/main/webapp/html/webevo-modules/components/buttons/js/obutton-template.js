...
function buildTemplateOButton(data, tempStyle, tempBG) {
	/*
	 * ThoaiVT split rgba from data 25/11/2016
	 */
	var hover_bg = "";
	if (data['obuttonBGHover'] && data['obuttonBGHover']['onmouseover']) {
		try {
			var context = data['obuttonBGHover']['onmouseover']['background-image'].toString();
			console.log(data['obuttonBGHover']['onmouseover']['background-image']);
			var n = context.indexOf("rgba");
			var dex = context.indexOf("),");
			hover_bg = context.substring(n, dex + 1);
		} catch (e) {
...
//			console.log(e);
	...
	template_obutton += '<div class="qb-component '+((data.obuttonAnimation) ? data.obuttonAnimation : "")+'" style="overflow:hidden; ' + tempStyle + '" ho-background="' + hover_bg + '">';
	template_obutton += '			<div class="layer-background" style="' + tempBG + '" ';
	// template_obutton += obuttonOtherAttributes;
	template_obutton += '></div>';
	template_obutton += '<a class="obutton-link" data-action-type="' + data.actionType + '" data-action-link="' + data.actionLink + '" data-action-link-target="' + data.actionLinkTarget + '" data-action-tracking="' + data.actionTracking + '" data-doc-id="' + data.dataDocId + '"';
	template_obutton += '>';
	...
	if(web.MV922==="ADMIN")
		template_obutton += '<li class="btnadmin-set-tolibrary"><a><i class="fa fa-bookmark-o"></i>Add to library</a></li>';
	...
	template_obutton += '<li class="btn-set-action"><a><i class="fa fa-link"></i>' + getLocalize("jsobt_title4") + '</a></li>';
	template_obutton += '<li class="btn-comp-duplicate"><a><i class="fa fa-copy"></i>' + getLocalize("setting_duplicate") + '</a></li>';
	template_obutton += '<li class="btn-comp-copy"><a><i class="icon-display"></i>' + 'Copy (Ctrl + C)' + '</a></li>';
