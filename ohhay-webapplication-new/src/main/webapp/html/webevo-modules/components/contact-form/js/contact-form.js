	...
	contactForm = {
		...
		toolEventListener: function(){
			...
			$(document).on('click','#qb-dlg-contact-form .item-save',function(){
				...
				var mailSend = $('#qb-dlg-contact-form .edit-email-revice-tool .input-email-recive').val().trim();
				if (mailSend === $('#qb-userlogin-mv903decrypt').val())
					mailSend = "";
				$('#qb-dlg-contact-form .form-body-tool .qb-form-email-recive').val(mailSend);
			...
			$(document).on('click','#qb-dlg-contact-form .form-menu-tool .sub-items-right-input-design .btn-set-input-style-bg',function(){
				...
				backGroundForm.setBackground(targetBox, 'F');
			});
			//input style effect
			...
			/*
			 * event choice size column
			 */
			
			$(document).on('click','#qb-dlg-contact-form .form-body-tool .function-form-field .btn-set-rowsize',function(){
				var el = $(this).closest('.form-group-ex').find("textarea");
				editText.selfRs = el;
				editText.open({
					callBack: function(result){
						el.attr("rows", result.value);
					},rowTextArea: true
				},$('#qb-dlg-contact-form .form-body-tool .qb-btn-submit-email span'));
			...
			$(document).on('click','#qb-dlg-contact-form .form-body-tool .qb-input-panel .form-group-ex[qb-input-type="radio"] .btn-form-field-edit',function(){
				...
				var cssSty = self.find("label").attr("style");
						...
						if(ss && ss.length > 0){
							for(i=0; i<ss.length; i++){
								if(ss[i] && ss[i].trim().length > 0){
									...
									htmlString += '<label style="'+cssSty+'" contenteditable="true">'+ss[i]+'</label>';
								}
							}
						}
			...
			$(document).on('click', '#qb-dlg-contact-form .form-body-tool .qb-contact-form-submit-content .function-button .btn-set-submit-font', function(event) {
				editText.open({
					callBack: function(result){
						$('#qb-dlg-contact-form .form-body-tool .qb-btn-submit-email > *').css(result.key, result.value);
					}
				},$('#qb-dlg-contact-form .form-body-tool .qb-btn-submit-email > *'));
			});
			...
			$(document).on('click', '#qb-dlg-contact-form .item-fontstyle-field-title', function(event) {
				...
				editText.open({
					callBack: function(result){
						$('#qb-dlg-contact-form .form-body-tool .form-group-ex .field-title span,#qb-dlg-contact-form .form-body-tool .form-group-ex .field-content label').css(result.key, result.value);
					}
				},$('#qb-dlg-contact-form .form-body-tool .form-group-ex .field-title span, .qb-form-send-mail .qb-form-send-mail-wrapper-panel .qb-input-panel .form-group-ex .field-content label'));
			});
			...
			$(document).on('click', '#qb-dlg-contact-form .item-fontstyle-panel-title', function(event) {
				...
				},$('#qb-dlg-contact-form .form-body-tool .qb-input-panel .panel-title'));
			});
			/*
		...
		add : function(grid, x, y, $self) { 
			...
			//widget.find('.btn-edit-contact-form').trigger('click');
			return widget;
		},
		/*
		...
		open: function(option){
			...
			var mailSend = $('#qb-dlg-contact-form .form-body-tool .qb-form-email-recive').val();
			if(mailSend==="")
				mailSend = $('#qb-userlogin-mv903decrypt').val();
			$('#qb-dlg-contact-form .edit-email-revice-tool .input-email-recive').val(mailSend);
		},
		/*
		...
		setUpPanelEdior: function(panel){
			...
			var typeEx = ""; 
			try{
				typeEx = panel.find('.form-group-ex').attr('qb-input-type');		
			}catch(e){
				console.log(e);
			}
			 ...
			 * add asterisk
			 */
			if(typeEx === 'text' || typeEx === 'textarea' || typeEx === 'checkbox' ||  typeEx === 'radio' ||  typeEx === 'select')
			{
			function_form_field +=			'<div title="Required field: On/Off" class="item btn-set-require">';
			function_form_field += 				'<i class="fa fa-asterisk"></i>';
			function_form_field +=			'</div>';
			}
			/*
			 * done asterisk
			...
			function_form_field_input +=			'<div title="Row column" class="item btn-set-rowsize">';
			function_form_field_input += 				'<i class="fa fa-pencil"></i>';
			function_form_field_input +=			'</div>';
			...
			panel.find('.form-group-ex').each(function(){
				if($(this).attr('qb-input-type') == 'text' || $(this).attr('qb-input-type') == 'textarea' ||  $(this).attr('qb-input-type') == 'checkbox' ||  $(this).attr('qb-input-type') == 'radio' ||  $(this).attr('qb-input-type') == 'select')
				{
					$(this).prepend(function_form_field_input);
					if($(this).attr('qb-input-type') != 'textarea'){
						$(this).find('.function-form-field .btn-set-rowsize').css("display","none");
					}
			...
			panel.droppable({
				...
				drop : function(e, ui) { 
					...
					panel.find('.form-group-ex:last .btn-set-require').css('background','#0088CC');
				}
			});
		...
		createTextBoxTemplate: function(){
			var template = 	'<div class="form-group-ex" qb-input-type="text" required-field="true">';
		...
		createTextAreaTemplate: function(){
			var template = 	'<div class="form-group-ex" qb-input-type="textarea" required-field="true">';
		...
		createRadioButtonTemplate: function(){
			...
			var template = 	'<div name="'+name+'" class="form-group-ex" qb-input-type="radio" required-field="true">';
		...
		createComboBoxTemplate: function(){
			var template = 	'<div class="form-group-ex" qb-input-type="select" required-field="true">';
			template += 	'	<div contenteditable="true" class="field-title "><span>Title</span></div>';
			template += 	'	<div class="field-content">';
			template += 	'		<select>';
		...
		createCheckBoxTemplate: function(){
			...
			var template = 	'<div name="'+name+'" class="form-group-ex" qb-input-type="checkbox" required-field="true">';
			template += 	'	<div contenteditable="true" class="field-title "><span>Title</span></div>';
			template += 	'	<div class="field-content">';
