		...
		registerSocket : function() {
			...
			try {
				...
				self.socket.on('reponseS3UploadBGRepeat', function(response) {
					...
					imgHtml += '		<div class="function-panel"><span class="remove-img"><img src="'+util.contextPath()+'/html/images/menu-close.png" /></span></div>';
					imgHtml += '	</div>';
//					$(imgHtml).insertAfter("#qb-dlg-bg-repeat-col .qb-my-image-panel .btn-add-img");
					$("#qb-dlg-bg-repeat-col .qb-my-image-panel .content-data-append").prepend(imgHtml);
				...
				self.socket.on('reponseS3UploadLibrary', function(response) {
					...
					imgHtml += "		<span class='remove-img'><img src='"+util.contextPath()+"/html/images/menu-close.png' /></span>";
					imgHtml += "	</div>";
					imgHtml += "</div>";
//					$(imgHtml).insertAfter(".qb-my-image-panel .btn-add-img");
//					$("#qb-dlg-album .qb-ohhay-image-panel")
					$(".qb-my-image-panel .content-data-append").prepend(imgHtml);
					$.ajaxSetup({
						beforeSend : function() {
						},
						complete : function() {
