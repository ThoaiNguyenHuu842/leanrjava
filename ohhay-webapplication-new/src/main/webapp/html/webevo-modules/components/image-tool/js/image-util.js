/**
 * @author Anph create date: 26/10/2015 save original source img always greater frame
 */
(function() {
	/*
	 * 
	...
	imageUtil = {
		...
		resizeSingleImageNinitCropbox : function($img, originalSrc, boxImg) {
			...
			img.onload = function() {
				...
				// set width to max width allow =
				// webConfig.CURRENT_WIDTH
				resizedW = (naW > webConfig.data.w) ? webConfig.data.w : naW,
				// new height = (original height / original width) x new
				// width
		...
		checkSameY : function(boxImg) {
			...
			for (var i = 0; i < arrSParRect.length; i++) {// loop all
				// sibling
	...
	imageTool = {
		actionSaveOnclick : function($box) {
			...
			if (mode == 'GALLERY') {
				...
				if (mode == 'ONE') {
					// init data ready save
					var $img = $box.find('img'),
					// checkbox keep original val
					keepOriginal = $box.find('.chkb-keep-original .keep-orginal')[0].checked,
					//
					...
					// keep original image to save: dont request nodejs cut this image
					if (keepOriginal) {
						data['img'][0] = $img.attr('src');
						componentModelManager.updateItemField($box.attr('qb-component-id'), WEB_PRO_DATA, data);
						// disable, hide lading, end mode upload
						$img.removeClass('cropbox-enable').data('cropbox').remove();
	...
	imageBGUtil = {
		...
		/*
		 * function($box : elem need set background, imgSrc : http source, type:{ G:gridbox, B: bigbox }, callBackSave : function after save)
		 */
		...
		saveCropboxBGNode : function(callback, $img) {
			...
			if (typeof crop != 'undefined') {
				...
				// imageController.bgFilterMap[self.stubId] =
				// this.targetBG.;
				if (!$('.qb-ohhay-ajax-content').is(":visible"))
					$('.qb-ohhay-ajax-content').show();
				imageController.socket.emit('cropImgAndSaveImageToS3', {
			...
			if (from === 'pub') {
				...
				$boxImg.find('.fullscreen-action.use-fullscreen').on('click', function() {
					if ($boxImg.hasClass('fullscreen')) {
						...
						self.launchFullScreen($boxImg[0]);
