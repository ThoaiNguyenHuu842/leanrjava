 ...
 * @author ThoaiNH
 * create Oct 01, 2015
 * update Nov 23, 2015: edit _update_container set new height only new height > current height
 * ThoaiVT update Aug 24, 2016: config undo manager
 */
(function(factory) {
    ...
    GridStack.prototype._update_container_height = function() {
        ...
        var newH =  this.grid.get_grid_height() * (this.opts.cell_height + this.opts.vertical_margin) - this.opts.vertical_margin;
        if(newH > this.container.height())
    ...
    GridStack.prototype._prepare_element = function(el) {
        ...
        var cell_w, cell_h, cell_x, cell_y;
        var on_start_moving = function(event, ui) {
        	$(this).find('iframe').css('pointer-events','none');
        	$('#content-wrapper .evo-hair-vertical-right,#content-wrapper .evo-hair-vertical-left').show();
        	$(this).parents('.content-box').find('.evo-hair-horizontal-top').show();
        	$(this).parents('.content-box').find('.evo-hair-horizontal-bottom').show();
        ...
        var undoresize = function(o, gsxs,gsys,gsws,gshs){
        	//event undo resize component
    		try{
	        	o.attr('data-gs-x', gsxs)
	            .attr('data-gs-y', gsys)
	            .attr('data-gs-width',gsws)
	            .attr('data-gs-height', gshs).removeAttr('style');
	    		console.log(gsxs,gsys,gsws,gshs);
    		}catch(e){
    			console.log(e);
    		}
    	};
        ...
        var on_end_moving = function(event, ui) {
        	//hide center line
            if (web.GRID_LINE != 'on')
            	$('.evo-grid-line-x-24').hide();
        	$(this).find('iframe').css('pointer-events','auto');
        	$('#content-wrapper .evo-hair').hide();
            ...
            try{
	            ...
	            var gsx = o.attr('data-gs-x');
	            var gsy = o.attr('data-gs-y');
	            console.log(gsx,gsy);
	            var gsw = o.attr('data-gs-width');
	            var gsh	= o.attr('data-gs-height');
	            
	            self._update_container_height();
	            self._trigger_change_event();
	
	            
	        	console.log("cell_width != gsw",cell_w, gsw);
	        	console.log("cell_height != gsh",cell_h, gsh);
	            if(ENABLE_UNDOMANAGER==1){
		        	if(cell_w != gsw || cell_h != gsh){
		            	undoMgr.add({
			                undo: function() {
			                	undoresize(o, c_x,c_y,c_w,c_h);
			                }, 
			                redo: function() {
			                	undoresize(o, c_x,c_y,gsw,gsh);
			             	}
			            });
        ...
        el.draggable(_.extend(this.opts.draggable, {
            ...
            drag: function(event, ui) {
                ...
                self._update_container_height();
                //show crosshair 
                var l = ui.position.left + $(this).width();
                var t = ui.position.top + $(this).height();
                $('#content-wrapper .evo-hair-vertical-left').css({'left':ui.position.left+'px'});
                $('#content-wrapper .evo-hair-vertical-right').css({'left':l+'px'});
                $('#content-wrapper .evo-hair-horizontal-top').css({'top':ui.position.top+'px'});
                $('#content-wrapper .evo-hair-horizontal-bottom').css({'top':t+'px'});
                
                
                cell_w = o.attr('data-gs-width');
                cell_h = o.attr('data-gs-height');
                cell_x = o.attr('data-gs-x');
                cell_y = o.attr('data-gs-y');
                var x2 = parseInt(x) + parseInt(cell_w);
                /*
                 * update 21/10/2016 - ThoaiNH: show center line when moving a component center of page
                 */
                if (web.GRID_LINE != 'on')
                {
                	//when moving into center coordinate
                	if(gridConfig.width - x2 == x)
                		$('.evo-grid-line-x-24').show();
                	else
                		$('.evo-grid-line-x-24').hide();
                }
            },
            ...
            resize: function(event, ui) {
            	var o = $(this);
            	var minSize = componentModelManager.getMinSize(ui.element);
            	//THOAINH: customize 12/18/2015, grid unit too small so the calculation is not right
                var x = ui.element.data('gx-x'),
                    y = ui.element.data('gx-y'),
                    width = Math.round(ui.size.width / cell_width),
                    height = Math.round(ui.size.height / cell_height);
                if(width < minSize.minW)
                	width = minSize.minW;
                if(height < minSize.minH)
                	height = minSize.minH;
                ...
                self._update_container_height();
                //show crosshair 
                var l = ui.position.left + ui.size.width;
                var t = ui.position.top + ui.size.height;
                $('#content-wrapper .evo-hair-vertical-left').css({'left':ui.position.left+'px'});
                $('#content-wrapper .evo-hair-vertical-right').css({'left':l+'px'});
                $('#content-wrapper .evo-hair-horizontal-top').css({'top':ui.position.top+'px'});
                $('#content-wrapper .evo-hair-horizontal-bottom').css({'top':t+'px'});
    ...
    var removeComponent = function(el){
    	try{
    		el.show();
    	}catch(e){
    		console.log(e);
