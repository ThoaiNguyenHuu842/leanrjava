	...
	componentModelManager = {
		...
		updateItemField : function(id, fieldName, value) {
			...
			try {
				...
				// componentModelManager.listComponent[id][fiel dName] + value :
				// value;
				
				// replace
				componentModelManager.listComponent[id][fieldName] = value;
				componentModelManager.updateDataButtonHover(fieldName,id);
				...
				var boxId = componentModelManager.listComponent[id][WEB_PRO_BOXID];
				// update status of contain box
				if(bigBoxModelManager.listBox[boxId][WEB_PRO_STT] == WEB_PRO_STT_NO_CHANGE)
					bigBoxModelManager.listBox[boxId][WEB_PRO_STT] = WEB_PRO_STT_UPDATE;
			} catch (e) {
				console.log(e);
			}
		...
		loadComponent : function(component) {
			...
			if (web.MOBILE_EDITOR == 'on') {
				...
				{
					// alert('on:'+component[WEB_PRO_MDATA][WEB_PRO_X]+","+component[WEB_PRO_MDATA][WEB_PRO_Y]+","+component[WEB_PRO_MDATA][WEB_PRO_W]+","+component[WEB_PRO_MDATA][WEB_PRO_H]);
					node = {
			...
			var grid = $(".content-box[qb-box-id='" + component[WEB_PRO_BOXID] + "'] .grid-stack ").data('gridstack');
			...
			case WEB_COMP_TYPE_TEXT:
				...
				if (web.MOBILE_EDITOR == 'on' && component[WEB_PRO_MDATA] && component[WEB_PRO_MDATA][WEB_PRO_DATA]) {
					...
					/*
					 * if (data["line-height"]) {
					 * $(widget.find(".otext-wrapper-content
			...
			case WEB_COMP_TYPE_BTN:
				...
				// load font scale for button
				if (web.MOBILE_EDITOR == 'on' && component[WEB_PRO_MDATA] && component[WEB_PRO_MDATA][WEB_PRO_DATA]) {
					var mData = JSON.parse(component[WEB_PRO_MDATA][WEB_PRO_DATA]);
					if (mData[WEB_PRO_FONTSCALE]) {
						$(widget.find(".obutton-link *").get().reverse()).each(function() {
							util.scaleFontSize($(this), parseInt(mData[WEB_PRO_FONTSCALE]));
							// $(this).css('line-height', '23px');
						});
					}
				}
			...
			if (web.MOBILE_EDITOR == 'on')
			{
				...
				else if(component[WEB_PRO_CSS] && component[WEB_PRO_CSS].length > 0)
					widget.find(".qb-component").attr('style', component[WEB_PRO_CSS]);
			...
			if (component[WEB_PRO_NAME] && component[WEB_PRO_NAME] != 'null') {
				...
				widget.find('li.btn-set-anchor label').html(getLocalize("jsmtp_title3")+" "+component[WEB_PRO_NAME]);
			} else {
				widget.find('li.btn-set-anchor label').html(getLocalize("jsmtp_title3")+" "+component[WEB_PRO_ID]);
			}
			// setup onload effect
			...
			if (component[WEB_PRO_DATA] && component[WEB_PRO_DATA]['writingMode'] && component[WEB_PRO_DATA]['writingMode'] === 'vertical') {
		 ...
		 * hover update data
		 */
		updateDataButtonHover: function(fieldName,id){
			try{
				if(fieldName == "bgCss"){
					var bgImage = componentModelManager.listComponent[id]["data"]["obuttonBGHover"]["onmouseout"]["background-image"];
					var bgColor = componentModelManager.listComponent[id]["data"]["obuttonBGHover"]["onmouseout"]["background-color"];
					var bgHv = componentModelManager.listComponent[id]["data"]["obuttonBGHover"]["onmouseover"]["background-image"];
				    var mouseOutDt = componentModelManager.listComponent[id]["data"]["obuttonBGHover"]["onmouseout"]; 
					if(bgHv!=undefined && bgHv!=""){
				    	var indLg = bgHv.indexOf("rgba",bgHv.indexOf("rgba")+4);
					    var linerGd = bgHv.substring(bgHv.indexOf("rgba"),indLg-1);
						var strLn = 'linear-gradient('+linerGd+','+linerGd+') ,'+((bgImage)!='inherit' ? bgImage :  'none')+' , '+((bgColor)!= 'inherit' ? bgColor : 'none');
						componentModelManager.listComponent[id]["data"]["obuttonBGHover"]["onmouseover"]["background-attachment"] = ((mouseOutDt["background-attachment"])!=undefined ? mouseOutDt["background-attachment"] : "");
						componentModelManager.listComponent[id]["data"]["obuttonBGHover"]["onmouseover"]["background-clip"] = ((mouseOutDt["background-clip"])!=undefined ? mouseOutDt["background-clip"] : "");
						componentModelManager.listComponent[id]["data"]["obuttonBGHover"]["onmouseover"]["background-origin"] = ((mouseOutDt["background-origin"])!=undefined ? mouseOutDt["background-origin"] : "");
						componentModelManager.listComponent[id]["data"]["obuttonBGHover"]["onmouseover"]["background-repeat"] = ((mouseOutDt["background-repeat"])!=undefined ? mouseOutDt["background-repeat"] : "");
						componentModelManager.listComponent[id]["data"]["obuttonBGHover"]["onmouseover"]["background-position"] = ((mouseOutDt["background-position"])!=undefined ? mouseOutDt["background-position"] : "");
						componentModelManager.listComponent[id]["data"]["obuttonBGHover"]["onmouseover"]["background-size"] = ((mouseOutDt["background-size"])!=undefined ? mouseOutDt["background-size"] : "");
						componentModelManager.listComponent[id]["data"]["obuttonBGHover"]["onmouseover"]["background-image"] = strLn;
					}
				}
			}catch(e){
				console.log("not set hover on component");
			}
		}
	}
}());