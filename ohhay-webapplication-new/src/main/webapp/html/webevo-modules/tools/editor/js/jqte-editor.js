...
(function($){		
	$.fn.jqte = function(options){
		...
		//font-family
		var fontfamilys = ["Open Sans","Roboto","arial","sans-serif","HelveticaNeue","Avenir Next","Dancing Script","Lato","Ubuntu","Akronim","Itim","Alegreya Sans","Kanit","Fira Sans","Crimson Text","Roboto Slab","Aachen","MyriadPro","Pattaya", "Bungee", "Bungee Shade", "Bungee Inline", "Arima Madurai", "Pangolin", "EB Garamond", "Pacifico", "Anton", "Lobster", "Alfa Slab One", "Sigmar One", "Bungee Outline", "Sriracha", "Monoton"];
...
//						thisElement.css("height",editor.outerHeight());
			...
			thisElement
				.bind("keydown keyup",function()
				{
					...
					$(this).height($(this)[0].scrollHeight);alert();
				...
				if (window.getSelection)
				{
					...
					if (selObj.anchorNode && selObj.getRangeAt)
					{
						...
						if(ancestor.childNodes.length > 3 && (ancestor.childNodes[1].tagName.toLowerCase()=="span" || ancestor.childNodes[1].tagName.toLowerCase()=="a") && ancestor.childNodes[0].data=='' && ancestor.childNodes[2].data==''){
							...
							if((selRange.startOffset + 1) == selRange.endOffset){
								console.log('---case 1 dong---');
								var stt = selRange.startOffset;
								if(ancestor.childNodes[stt].childNodes[0].nodeName.toLowerCase() == 'span'){
									$(ancestor.childNodes[stt].childNodes[0]).attr('colorflag','');
								}else{
									documentFragment = ancestor.childNodes[stt].innerHTML;
									newElement = '<span colorflag="">'+documentFragment+'</span>';
									ancestor.childNodes[stt].innerHTML = newElement;
								}
							}else{
								console.log('---case n dong---');
			...
			$(document).on('mousedown','.otext-wrapper-content[contenteditable="true"], .grid-style-onedit .obutton-label[contenteditable="true"], .evo-editor', function(){
				...
				if($('.qb-component').find('[jqte-setflag]').length){
					if($('.qb-component').find('[jqte-setflag]').attr('style') || $('.qb-component').find('[jqte-setflag]').attr('data-action-type')){
						if($('.qb-component').find('[jqte-setflag]').find('p').length ||
							$('.qb-component').find('[jqte-setflag]').find('h1').length ||
							$('.qb-component').find('[jqte-setflag]').find('h2').length ||
							$('.qb-component').find('[jqte-setflag]').find('h3').length ||
							$('.qb-component').find('[jqte-setflag]').find('h4').length ||
							$('.qb-component').find('[jqte-setflag]').find('h5').length ||
							$('.qb-component').find('[jqte-setflag]').find('h6').length){
				...
				//$(".otext-wrapper-content br").remove();
				//$(".otext-wrapper-content i.fa:empty").remove();
				
				 if ($('#qb-dlg-color-picker').closest('.ui-dialog').is(':visible') === true) {
				 	colorPicker.close();
				 }
				 if ($('#qb-dlg-action-option').closest('.ui-dialog').is(':visible') === true) {
					 actionOption.close();
				 }
			});
			...
			function getValForEditor(ob_class){
				...
				function styleElement(name){
					...
					if(name==='fontsize'){
						...
						thisSelection.css('color',fontcolor.data('val'));
//						console.log('then tho khung');
...
//						affectStyleAround(thisSelection,'text-shadow:'+val);
					}else if(name==='link'){
						var link = linkinput.val();
						//changTagSelection('a');
						insertLink();
